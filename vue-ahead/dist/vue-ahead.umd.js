	...
	var script$7 = {
		...
		methods: {
			...
			clear: function () {
				...
				if (oldItems && oldItems.length > 0) {
					this.$emit("selection", {
						type: "clear",
						items: [],
						removed: oldItems.map(function (item) {
			...
			handleItemSelection: function (evt, key) {
				...
				this.$emit("selection", {
					type: "add",
					items: this.selection.items.map(function (item) {
						return item["type"] === "created" ? item : item["src"]
					}),
					replaced: last,
					value: this.value,
