...
exports.indexEngine = options => {
  ...
  const tokenize = (phrase, scopeToken) => {
    ...
    return phrase.split(delimiter).filter(val => val).map(val => val.trim().toLowerCase());
  };
  ...
  const findDocs = tokens => {
    ...
    let matchedTokens = [];
    let cacheKey = tokens.length > 1 ? tokens.slice(0, -1).join(delimiter) : tokens[0];
    let {
      docs,
      start
    } = _cache.hit(cacheKey) || defaultCacheHit; // if we can locate a prefix previously being searched, use it as the start point
    ...
    for (let i = start; i < tokens.length; i++) {
      ...
      let {
        ...
        keyword,
        matches
      } = _trie.find(token) || {}; // if not found, we will have at most partial matching, and we shall return null 
      ...
      token = keyword;
      let next = {};
      let hasDoc = false; // transforming the keywords list into doc+position list
    ...
    if (Array.isArray(tokens) && tokens.length > 0) {
      for (let i = 0; i < tokens.length; i++) {
        ...
        if (_rIndex.hasOwnProperty(word) && Array.isArray(_rIndex[word])) {
          _rIndex[word] = Array.prototype.filter.call(_rIndex[word], item => item.docID !== docID);
        }
      }
    }
  ...
  const _debug = () => {
    Object.keys(_rIndex).forEach(key => console.info(key));
