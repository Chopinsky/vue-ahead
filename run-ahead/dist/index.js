...
exports.runahead = (options, data, initDataType) => {
  ...
  const send = (remote, type, callback) => {
    ...
    provider(remote).then(resp => {
      // format data if a formatter is passed
      let data = typeof dataParser === "function" ? dataParser(resp.data, remoteType.query) : resp.data; // now add newly obtained data to the index search engine
